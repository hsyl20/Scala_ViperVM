...
trait CCode extends Block with Scope {
   ...
   implicit def sym2var(s:Symbol): Variable = {
      getSymbol(s.name) match {
         ...
         case _ => sys.error("Invalid symbol %s".format(s.name))
      }
   }
   ...
   implicit def symex(s:Symbol) = new {
      ...
      def :- (t:CType): Variable = {
         getSymbol(s.name) match {
            case Some(v:Variable) => if (v.typ != t) sys.error("Invalid type ascription") else v
            case None => {
               val v = Variable(t)
               declare(v)
               addSymbol(s.name, v)
      ...
      def :- (vt:VarType): Variable = {
         getSymbol(s.name) match {
            case Some(v:Variable) => if (v.typ != vt.typ || v.space != vt.space) sys.error("Invalid ascription") else v
            case None => {
